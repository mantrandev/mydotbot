...
Use this as the operating guide for the personal Claude profile (`~/.claude`).
...
- `~/Desktop/projects/mydotbot/ai/claude/CLAUDE.md` is this profile's rules source.
...
- `~/Desktop/projects/mydotbot/ai/skills-claude/` is this profile's generated skills root, selected by `ai/skill-profiles.conf`.
...
- When updating this profile's rules, change `~/Desktop/projects/mydotbot/ai/claude/CLAUDE.md` only.
...
- Before writing any standalone HTML, follow the `blueprint-html` skill: start from its bundled `assets/template.html`, reuse its component blocks and design tokens, and keep its single blueprint theme.
...
- This profile commits as `mantrandev <mantrandev@gmail.com>` on GitHub repos.
- A GitLab repo does not belong in this profile — switch to `claude-company` instead of committing here.
